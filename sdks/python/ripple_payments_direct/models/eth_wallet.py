...
class EthWallet(BaseModel):
    ...
    """ # noqa: E501
    wallet_address: Annotated[str, Field(min_length=42, strict=True, max_length=42)] = Field(description="Receiving wallet address at the exchange", alias="walletAddress")
    crypto_institution_name: StrictStr = Field(description="Name of the crypto exchange", alias="cryptoInstitutionName")
    network: Optional[StrictStr] = Field(default='ETHEREUM', description="The default name of the blockchain network for this instrument")
    ...
    def wallet_address_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^0x[a-fA-F0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^0x[a-fA-F0-9]+$/")
        return value
    ...
    def supported_corridors_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['ETH_USDT', 'ETH_USDC', 'ETH_RLUSD']):
                raise ValueError("each list item must be one of ('ETH_USDT', 'ETH_USDC', 'ETH_RLUSD')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "network": obj.get("network") if obj.get("network") is not None else 'ETHEREUM',
