...
class PaymentFilterV2(BaseModel):
    """
    ...
    payment_states: Optional[List[StrictStr]] = Field(default=None, description="Array of payment states like `EXECUTING` or `COMPLETED`.", alias="paymentStates")
    filter_range_type: Optional[StrictStr] = Field(default=None, description="The type of timestamp to use when filtering payments by a date and time range.  Supported values include: - `PAYMENT_CREATION` - `PAYMENT_EXPIRY` - `PAYMENT_STATUS_LAST_UPDATED`  If another value is provided, the filter is rejected with a validation error. ", alias="filterRangeType")
    ...
    beneficiary_identity_ids: Optional[List[StrictStr]] = Field(default=None, description="To get all payments made to one or more beneficiaries, specify the identity ID(s) of those beneficiaries in this field. You can find the identity ID using the `GET /identities` operation.", alias="beneficiaryIdentityIds")
    internal_id: Optional[Annotated[str, Field(strict=True, max_length=36)]] = Field(default=None, description="Specify an `internalId` to get payments associated with them. A prefix match will be executed.", alias="internalId")
    beneficiary_identity_nickname: Optional[Annotated[str, Field(strict=True, max_length=256)]] = Field(default=None, description="To get all payments made to a beneficiary, specify the nickname of that beneficiary in this field. You can find the nickname using the `GET /identities` operation. The nickname is matched by the identity service, and the payments of every beneficiary it matches are returned.", alias="beneficiaryIdentityNickname")
