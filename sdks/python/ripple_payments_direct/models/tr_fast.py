...
class TrFast(BaseModel):
    ...
    iban: Annotated[str, Field(min_length=26, strict=True, max_length=26)] = Field(description="Turkey IBAN (26 characters, starting with TR)")
    bank_code: Annotated[str, Field(min_length=1, strict=True, max_length=15)] = Field(description="SWIFT/BIC code of the identity's bank. Named bankCode for v2 ADR compatibility; maps to BICFI in ISO 20022.", alias="bankCode")
    __properties: ClassVar[List[str]] = ["bankName", "iban", "bankCode"]
    ...
    def iban_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^TR[0-9]{24}$", value):
            raise ValueError(r"must validate the regular expression /^TR[0-9]{24}$/")
        return value
