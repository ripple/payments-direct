...
class JpZengin(BaseModel):
    ...
    account_holder_name: Annotated[str, Field(min_length=1, strict=True, max_length=100)] = Field(description="Name of the account holder in Japanese Katakana characters", alias="accountHolderName")
    bank_code: Annotated[str, Field(min_length=1, strict=True, max_length=16)] = Field(description="Zengin-Net/BOJ-assigned 4-digit Japanese bank code (e.g. 0001 Mizuho Bank, 0005 MUFG Bank). See the Bank Codes resource in Ripple Docs for the authoritative list of supported values.", alias="bankCode")
    branch_code: Annotated[str, Field(min_length=1, strict=True, max_length=50)] = Field(description="Bank-assigned branch code identifying the specific branch", alias="branchCode")
    account_number: Annotated[str, Field(min_length=1, strict=True, max_length=45)] = Field(description="The identity's bank account number", alias="accountNumber")
    account_type: StrictStr = Field(description="The identity's account type", alias="accountType")
    __properties: ClassVar[List[str]] = ["bankName", "accountHolderName", "bankCode", "branchCode", "accountNumber", "accountType"]
    ...
    def branch_code_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[A-Za-z0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^[A-Za-z0-9]+$/")
        return value
