...
class IndividualIdentityIdentityDocumentsInner(BaseModel):
    """
    ...
    id_type: StrictStr = Field(description="The type of identification document used to identify the identity.  - `ALIEN_REGISTRATION` — Alien Registration Number   - `CUSTOMER_ID` — Customer Identification Number   - `DRIVERS_LICENSE` — Driver’s License Number   - `PASSPORT` — Passport Number   - `EMPLOYEE_ID` — Employee ID   - `NATIONAL_ID_NUMBER` — National ID   - `SSN` — Social Security Number   - `TAX_ID` — Tax ID ", alias="idType")
    __properties: ClassVar[List[str]] = ["idNumber", "idType"]

    @field_validator('id_number')
    def id_number_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^(?![ .\'-\/])(?!.*[ .\'-\/]{2})([A-Za-z0-9 .\'-\/]+)(?<![ .\'-\/])\/?$", value):
            raise ValueError(r"must validate the regular expression /^(?![ .'-\/])(?!.*[ .'-\/]{2})([A-Za-z0-9 .'-\/]+)(?<![ .'-\/])\/?$/")
        return value
