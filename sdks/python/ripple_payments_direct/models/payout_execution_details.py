...
class PayoutExecutionDetails(BaseModel):
    """
    ...
    """ # noqa: E501
    payment_rail_used: Optional[StrictStr] = Field(default=None, description="The payment rail or network used to execute the payout (e.g., FEDWIRE, ACH, RTP_US, SPEI, FPS_UK). ", alias="paymentRailUsed")
    payout_start_time: Optional[datetime] = Field(default=None, description="The time at which the payout execution started, specified in UTC. ", alias="payoutStartTime")
    payout_end_time: Optional[datetime] = Field(default=None, description="The timestamp when the payout execution completed or was last updated, as reported by the payout partner. Specified in UTC using ISO 8601 format. For in-progress payouts, this may represent the last status update time. ", alias="payoutEndTime")
    tracking_references: Optional[List[TrackingReference]] = Field(default=None, description="A list of tracking references for this transaction, such as transaction IDs and confirmation numbers. ", alias="trackingReferences")
    __properties: ClassVar[List[str]] = ["paymentRailUsed", "payoutStartTime", "payoutEndTime", "trackingReferences"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in tracking_references (list)
        _items = []
        if self.tracking_references:
            for _item_tracking_references in self.tracking_references:
                if _item_tracking_references:
                    _items.append(_item_tracking_references.to_dict())
            _dict['trackingReferences'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "paymentRailUsed": obj.get("paymentRailUsed"),
            "payoutStartTime": obj.get("payoutStartTime"),
            "payoutEndTime": obj.get("payoutEndTime"),
            "trackingReferences": [TrackingReference.from_dict(_item) for _item in obj["trackingReferences"]] if obj.get("trackingReferences") is not None else None
        })
        return _obj
