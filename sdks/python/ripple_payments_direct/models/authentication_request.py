...
class AuthenticationRequest(BaseModel):
    ...
    """ # noqa: E501
    client_id: StrictStr = Field(description="The client ID associated with a specific set of API credentials.")
    client_secret: StrictStr = Field(description="The client secret associated with a specific set of API credentials.")
    audience: StrictStr = Field(description="The value of the `audience` field is based on [URN](https://en.wikipedia.org/wiki/Uniform_Resource_Name) syntax.  **Format**: `urn:ripplexcurrent-{ENVIRONMENT_STRING}:{YOUR_TENANT_ID}`  * The first component is `urn:ripplenetxcurrent-`. * The second component refers to the environment you want to access. * The third component is your tenant ID. Ripple integration engineers provide this component during training.  | Environment | Environment string | Description | | --- | --- | --- | | Test | `test` | Test environment with simulated partners and simulated currency. | | Production | `prod` | Production environment for Ripple's internal services. |  **Example**: `urn:ripplexcurrent-test:{YOUR_TENANT_ID}` ")
    grant_type: StrictStr = Field(description="Set the grant-type for this client credentials request. This must be set to `client_credentials`.")
    __properties: ClassVar[List[str]] = ["client_id", "client_secret", "audience", "grant_type"]
