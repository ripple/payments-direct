...
class ArInterbanking(BaseModel):
    """
    ...
    """ # noqa: E501
    bank_name: Annotated[str, Field(min_length=2, strict=True, max_length=140)] = Field(description="The name of the identity's bank. Must begin and end with a letter or digit, and consecutive identical punctuation characters are not permitted.", alias="bankName")
    bank_code: Annotated[str, Field(min_length=3, strict=True, max_length=3)] = Field(description="BCRA-assigned 3-digit Argentine bank code. See the Bank Codes resource in Ripple Docs for the authoritative list of supported values.", alias="bankCode")
    account_number: Annotated[str, Field(min_length=22, strict=True, max_length=22)] = Field(description="The identity's account number (CBU or CVU)", alias="accountNumber")
    ...
    def account_number_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^\d{22}$", value):
            raise ValueError(r"must validate the regular expression /^\d{22}$/")
        return value
