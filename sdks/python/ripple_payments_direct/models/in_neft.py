...
class InNeft(BaseModel):
    ...
    ifsc_code: Annotated[str, Field(min_length=11, strict=True, max_length=11)] = Field(description="The 11-character IFSC code of the identity's bank branch", alias="ifscCode")
    account_number: Annotated[str, Field(min_length=4, strict=True, max_length=45)] = Field(description="The identity's bank account number", alias="accountNumber")
    __properties: ClassVar[List[str]] = ["bankName", "ifscCode", "accountNumber"]
    ...
    def ifsc_code_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[A-Z]{4}0[A-Z0-9]{6}$", value):
            raise ValueError(r"must validate the regular expression /^[A-Z]{4}0[A-Z0-9]{6}$/")
        return value
