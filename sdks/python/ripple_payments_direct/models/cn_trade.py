...
class CnTrade(BaseModel):
    ...
    """ # noqa: E501
    bank_name: Annotated[str, Field(min_length=2, strict=True, max_length=140)] = Field(description="Beneficiary Bank Name. Looked up from bankCode.", alias="bankName")
    bank_code: Annotated[str, Field(min_length=1, strict=True, max_length=32)] = Field(description="Ripple Bank Code for the destination bank. Use the Ripple Bank Codes lookup at docs.ripple.com to find the correct code.", alias="bankCode")
    branch_name: Annotated[str, Field(min_length=1, strict=True, max_length=200)] = Field(description="Beneficiary Branch Name (Hanzi). Branch name in Chinese Hanzi characters.", alias="branchName")
    branch_province: Annotated[str, Field(min_length=2, strict=True, max_length=6)] = Field(description="Beneficiary Bank Branch Province. Province/region codes — 34 values published on Docs (e.g., 110000=北京, 310000=上海).", alias="branchProvince")
    branch_city: Annotated[str, Field(min_length=2, strict=True, max_length=6)] = Field(description="Beneficiary Bank Branch City. City codes — 700+ values published on Docs (e.g., 110000=北京, 130100=石家庄).", alias="branchCity")
    account_number: Annotated[str, Field(min_length=1, strict=True, max_length=45)] = Field(description="Beneficiary Account Number.", alias="accountNumber")
    account_holder_name: Annotated[str, Field(min_length=1, strict=True, max_length=140)] = Field(description="Beneficiary Account Holder Name (Hanzi). Account name in Chinese Hanzi characters.", alias="accountHolderName")
    __properties: ClassVar[List[str]] = ["bankName", "bankCode", "branchName", "branchProvince", "branchCity", "accountNumber", "accountHolderName"]
    ...
    def branch_name_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[\p{L}\p{N} .,\'&\/()-]+$", value):
            raise ValueError(r"must validate the regular expression /^[\p{L}\p{N} .,'&\/()-]+$/")
        return value

    @field_validator('branch_province')
    def branch_province_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^[0-9]+$/")
        return value

    @field_validator('branch_city')
    def branch_city_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[0-9]+$", value):
            raise ValueError(r"must validate the regular expression /^[0-9]+$/")
        return value
    ...
    def account_holder_name_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[\p{L}\p{N} .,\'&\/()-]+$", value):
            raise ValueError(r"must validate the regular expression /^[\p{L}\p{N} .,'&\/()-]+$/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "branchProvince": obj.get("branchProvince"),
            "branchCity": obj.get("branchCity"),
