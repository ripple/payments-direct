...
class TronWallet(BaseModel):
    ...
    """ # noqa: E501
    wallet_address: Annotated[str, Field(min_length=34, strict=True, max_length=34)] = Field(description="Receiving wallet address at the exchange", alias="walletAddress")
    crypto_institution_name: StrictStr = Field(description="Name of the crypto exchange", alias="cryptoInstitutionName")
    network: Optional[StrictStr] = Field(default='TRON', description="The default name of the blockchain network for this instrument")
    supported_corridors: Optional[List[StrictStr]] = Field(default=None, description="The list of supported corridors for the payout method", alias="supportedCorridors")
    __properties: ClassVar[List[str]] = ["walletAddress", "cryptoInstitutionName", "network", "supportedCorridors"]

    @field_validator('wallet_address')
    def wallet_address_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^T[1-9A-HJ-NP-Za-km-z]+$", value):
            raise ValueError(r"must validate the regular expression /^T[1-9A-HJ-NP-Za-km-z]+$/")
        return value

    @field_validator('supported_corridors')
    def supported_corridors_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['TRON_USDT']):
                raise ValueError("each list item must be one of ('TRON_USDT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "cryptoInstitutionName": obj.get("cryptoInstitutionName"),
            "network": obj.get("network") if obj.get("network") is not None else 'TRON',
            "supportedCorridors": obj.get("supportedCorridors")
        })
        return _obj
