...
class AeIpi(BaseModel):
    ...
    iban: Annotated[str, Field(min_length=23, strict=True, max_length=23)] = Field(description="The International Bank Account Number (IBAN) of the identity's account")
    __properties: ClassVar[List[str]] = ["bankName", "iban"]
    ...
    def iban_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^AE[0-9]{21}$", value):
            raise ValueError(r"must validate the regular expression /^AE[0-9]{21}$/")
        return value
