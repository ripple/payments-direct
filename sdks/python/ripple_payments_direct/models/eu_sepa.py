...
class EuSepa(BaseModel):
    ...
    iban: Annotated[str, Field(min_length=15, strict=True, max_length=34)] = Field(description="The International Bank Account Number (IBAN) of the identity's account")
    ...
    def iban_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[A-Z]{2}\d{2}[A-Z0-9]{11,30}$", value):
            raise ValueError(r"must validate the regular expression /^[A-Z]{2}\d{2}[A-Z0-9]{11,30}$/")
        return value
