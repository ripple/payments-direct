/*
Payments Direct API

Use the Payments Direct API to get quotes, create and manage payments, and manage originator and beneficiary identities.  ## API environments  The Payments Direct API offers the following environments:  | <div style=\"width:90px\">Environment</div>  | Base URL                      | Description                               | | ------------------------------------------ | ----------------------------- | ----------------------------------------- | | UAT                                       | `https://api.test.ripple.com` | UAT environment with simulated currency. | | Production                                 | `https://api.ripple.com`      | Production environment                    |  ## API authentication  All {% $env.PUBLIC_VAR_RPD %} API operations require a Bearer access token specific to the environment you're using. Ripple provides a secure model for authentication and authorization by providing access tokens scoped for a set of credentials.  ### Generate client ID and client secret  You will need your _client ID_ and _client secret_ to obtain an access token.  If you do not already have your client ID and client secret, do the following:  1. Log into the Ripple Payments UI. 2. In the left navigation menu, click **Settings**. 3. Under **Administration**, click **API Credentials**. 4. In the dropdown list next to the page title, select the access environment. For example, to provision credentials for the test environment, select **UAT** from the dropdown list. 5. In the upper right corner of the page, click **New Credential**. 6. Click **Save and Generate Key**.  **Caution:** The *client secret* is displayed only once when you are creating new credentials. You cannot retrieve the secret after exiting this page. Copy and store the client secret securely and share it with authorized individuals in accordance with your organization's security policy.  You can now use the client ID and client secret to generate access tokens using the [Request an access token](/products/payments-direct-2/api-docs/payments-direct-api/payments-direct-2-api/authentication/authenticate) operation.  ### Request an access token  To get an access token, use the [Request an access token](/products/payments-direct-2/api-docs/payments-direct-api/payments-direct-2-api/authentication/authenticate) operation with your `client_id` and `client_secret`. The response contains a token in the `access_token` field.  We recommend rotating your API credentials at regular intervals according to your organization's security policy.  **Note**: Authentication tokens are not a fixed length and can vary, avoid validating tokens based on character length. 

API version: 2026.04
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package paymentsdirect

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the GetFinancialInstrumentResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetFinancialInstrumentResponse{}

// GetFinancialInstrumentResponse This object contains the details of a financial instrument and optional identity if requested.
type GetFinancialInstrumentResponse struct {
	FinancialInstrument RippleFinancialInstrumentEntry `json:"financialInstrument"`
	Identity *GetIdentityResponseV3 `json:"identity,omitempty"`
}

type _GetFinancialInstrumentResponse GetFinancialInstrumentResponse

// NewGetFinancialInstrumentResponse instantiates a new GetFinancialInstrumentResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetFinancialInstrumentResponse(financialInstrument RippleFinancialInstrumentEntry) *GetFinancialInstrumentResponse {
	this := GetFinancialInstrumentResponse{}
	this.FinancialInstrument = financialInstrument
	return &this
}

// NewGetFinancialInstrumentResponseWithDefaults instantiates a new GetFinancialInstrumentResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetFinancialInstrumentResponseWithDefaults() *GetFinancialInstrumentResponse {
	this := GetFinancialInstrumentResponse{}
	return &this
}

// GetFinancialInstrument returns the FinancialInstrument field value
func (o *GetFinancialInstrumentResponse) GetFinancialInstrument() RippleFinancialInstrumentEntry {
	if o == nil {
		var ret RippleFinancialInstrumentEntry
		return ret
	}

	return o.FinancialInstrument
}

// GetFinancialInstrumentOk returns a tuple with the FinancialInstrument field value
// and a boolean to check if the value has been set.
func (o *GetFinancialInstrumentResponse) GetFinancialInstrumentOk() (*RippleFinancialInstrumentEntry, bool) {
	if o == nil {
		return nil, false
	}
	return &o.FinancialInstrument, true
}

// SetFinancialInstrument sets field value
func (o *GetFinancialInstrumentResponse) SetFinancialInstrument(v RippleFinancialInstrumentEntry) {
	o.FinancialInstrument = v
}

// GetIdentity returns the Identity field value if set, zero value otherwise.
func (o *GetFinancialInstrumentResponse) GetIdentity() GetIdentityResponseV3 {
	if o == nil || IsNil(o.Identity) {
		var ret GetIdentityResponseV3
		return ret
	}
	return *o.Identity
}

// GetIdentityOk returns a tuple with the Identity field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetFinancialInstrumentResponse) GetIdentityOk() (*GetIdentityResponseV3, bool) {
	if o == nil || IsNil(o.Identity) {
		return nil, false
	}
	return o.Identity, true
}

// HasIdentity returns a boolean if a field has been set.
func (o *GetFinancialInstrumentResponse) HasIdentity() bool {
	if o != nil && !IsNil(o.Identity) {
		return true
	}

	return false
}

// SetIdentity gets a reference to the given GetIdentityResponseV3 and assigns it to the Identity field.
func (o *GetFinancialInstrumentResponse) SetIdentity(v GetIdentityResponseV3) {
	o.Identity = &v
}

func (o GetFinancialInstrumentResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetFinancialInstrumentResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["financialInstrument"] = o.FinancialInstrument
	if !IsNil(o.Identity) {
		toSerialize["identity"] = o.Identity
	}
	return toSerialize, nil
}

func (o *GetFinancialInstrumentResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"financialInstrument",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetFinancialInstrumentResponse := _GetFinancialInstrumentResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varGetFinancialInstrumentResponse)

	if err != nil {
		return err
	}

	*o = GetFinancialInstrumentResponse(varGetFinancialInstrumentResponse)

	return err
}

type NullableGetFinancialInstrumentResponse struct {
	value *GetFinancialInstrumentResponse
	isSet bool
}

func (v NullableGetFinancialInstrumentResponse) Get() *GetFinancialInstrumentResponse {
	return v.value
}

func (v *NullableGetFinancialInstrumentResponse) Set(val *GetFinancialInstrumentResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableGetFinancialInstrumentResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableGetFinancialInstrumentResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetFinancialInstrumentResponse(val *GetFinancialInstrumentResponse) *NullableGetFinancialInstrumentResponse {
	return &NullableGetFinancialInstrumentResponse{value: val, isSet: true}
}

func (v NullableGetFinancialInstrumentResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetFinancialInstrumentResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


