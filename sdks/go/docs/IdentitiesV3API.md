...
func main() {
	internalId := "customer-12345-uuid" // string | The Internal ID of the identity to retrieve.

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.IdentitiesV3API.GetIdentityByInternalId(context.Background(), internalId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `IdentitiesV3API.GetIdentityByInternalId``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetIdentityByInternalId`: GetIdentityResponseV3
	fmt.Fprintf(os.Stdout, "Response from `IdentitiesV3API.GetIdentityByInternalId`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetIdentityByInternalIdRequest struct via the builder pattern
