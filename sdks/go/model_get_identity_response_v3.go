/*
Payments Direct API

Use the Payments Direct API to get quotes, create and manage payments, and manage originator and beneficiary identities.  ## API environments  The Payments Direct API offers the following environments:  | <div style=\"width:90px\">Environment</div>  | Base URL                      | Description                               | | ------------------------------------------ | ----------------------------- | ----------------------------------------- | | UAT                                       | `https://api.test.ripple.com` | UAT environment with simulated currency. | | Production                                 | `https://api.ripple.com`      | Production environment                    |  ## API authentication  All {% $env.PUBLIC_VAR_RPD %} API operations require a Bearer access token specific to the environment you're using. Ripple provides a secure model for authentication and authorization by providing access tokens scoped for a set of credentials.  ### Generate client ID and client secret  You will need your _client ID_ and _client secret_ to obtain an access token.  If you do not already have your client ID and client secret, do the following:  1. Log into the Ripple Payments UI. 2. In the left navigation menu, click **Settings**. 3. Under **Administration**, click **API Credentials**. 4. In the dropdown list next to the page title, select the access environment. For example, to provision credentials for the test environment, select **UAT** from the dropdown list. 5. In the upper right corner of the page, click **New Credential**. 6. Click **Save and Generate Key**.  **Caution:** The *client secret* is displayed only once when you are creating new credentials. You cannot retrieve the secret after exiting this page. Copy and store the client secret securely and share it with authorized individuals in accordance with your organization's security policy.  You can now use the client ID and client secret to generate access tokens using the [Request an access token](#operation/authenticate) operation.  ### Request an access token  To get an access token, use the [Request an access token](#operation/authenticate) operation with your `client_id` and `client_secret`. The response contains a token in the `access_token` field.  We recommend rotating your API credentials at regular intervals according to your organization's security policy.  **Note**: Authentication tokens are not a fixed length and can vary, avoid validating tokens based on character length. 

API version: 2026.03
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package paymentsdirect

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the GetIdentityResponseV3 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetIdentityResponseV3{}

// GetIdentityResponseV3 struct for GetIdentityResponseV3
type GetIdentityResponseV3 struct {
	// Server-generated unique identifier of the identity.
	IdentityId *string `json:"identityId,omitempty"`
	// The state of the identity   * ACTIVE   * DEACTIVATED 
	IdentityState *string `json:"identityState,omitempty"`
	// Optional human-readable alias set by the client.
	NickName *string `json:"nickName,omitempty"`
	// Optional labels used to categorize or filter identities.
	Tags []string `json:"tags,omitempty"`
	// List of payout methods where this identity is considered valid. Use this to indicate which payout methods (for example, US_ACH, EU_SEPA) the identity can be used with in payments.
	ValidatePayoutRails []string `json:"validatePayoutRails,omitempty"`
	// Sequential version number. Each successful PUT creates a higher version.
	Version *string `json:"version,omitempty"`
	// Schema version used to validate this identity, for example 1.0.0.
	SchemaVersion *string `json:"schemaVersion,omitempty"`
	// RFC 3339 timestamp when the identity was created.
	CreatedAt *time.Time `json:"createdAt,omitempty"`
	// RFC 3339 timestamp when the identity was last updated.
	UpdatedAt *time.Time `json:"updatedAt,omitempty"`
	// The type of the identity
	IdentityType string `json:"identityType"`
	// The payment role of the identity
	PaymentRole string `json:"paymentRole"`
	// Client-provided unique identifier for idempotency and deduplication. Required for ORIGINATOR identities; optional for BENEFICIARY identities. Must be unique across all active identities in your organization. Duplicate values will result in a 409 Conflict error.  This is your own reference key, not the originator's account number. Use `originatorAccountNumber` for that. 
	InternalId *string `json:"internalId,omitempty"`
	// The originator's account number, or your customer identifier for the originator. Required for ORIGINATOR identities on USD payments to China (`CN_CFXPS`); optional elsewhere. Only sent to payout partners for ORIGINATOR identities.  Must be unique across all active identities in your organization, including BENEFICIARY identities. Duplicate values return a 409 Conflict error.  Corridor length and character limits apply to this value but are not enforced on this field. See the corridor's Integration resources page. 
	OriginatorAccountNumber *string `json:"originatorAccountNumber,omitempty"`
	Business *BusinessIdentity `json:"business,omitempty"`
	Individual *IndividualIdentity `json:"individual,omitempty"`
}

type _GetIdentityResponseV3 GetIdentityResponseV3

// NewGetIdentityResponseV3 instantiates a new GetIdentityResponseV3 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetIdentityResponseV3(identityType string, paymentRole string) *GetIdentityResponseV3 {
	this := GetIdentityResponseV3{}
	this.IdentityType = identityType
	this.PaymentRole = paymentRole
	return &this
}

// NewGetIdentityResponseV3WithDefaults instantiates a new GetIdentityResponseV3 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetIdentityResponseV3WithDefaults() *GetIdentityResponseV3 {
	this := GetIdentityResponseV3{}
	return &this
}

// GetIdentityId returns the IdentityId field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetIdentityId() string {
	if o == nil || IsNil(o.IdentityId) {
		var ret string
		return ret
	}
	return *o.IdentityId
}

// GetIdentityIdOk returns a tuple with the IdentityId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetIdentityIdOk() (*string, bool) {
	if o == nil || IsNil(o.IdentityId) {
		return nil, false
	}
	return o.IdentityId, true
}

// HasIdentityId returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasIdentityId() bool {
	if o != nil && !IsNil(o.IdentityId) {
		return true
	}

	return false
}

// SetIdentityId gets a reference to the given string and assigns it to the IdentityId field.
func (o *GetIdentityResponseV3) SetIdentityId(v string) {
	o.IdentityId = &v
}

// GetIdentityState returns the IdentityState field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetIdentityState() string {
	if o == nil || IsNil(o.IdentityState) {
		var ret string
		return ret
	}
	return *o.IdentityState
}

// GetIdentityStateOk returns a tuple with the IdentityState field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetIdentityStateOk() (*string, bool) {
	if o == nil || IsNil(o.IdentityState) {
		return nil, false
	}
	return o.IdentityState, true
}

// HasIdentityState returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasIdentityState() bool {
	if o != nil && !IsNil(o.IdentityState) {
		return true
	}

	return false
}

// SetIdentityState gets a reference to the given string and assigns it to the IdentityState field.
func (o *GetIdentityResponseV3) SetIdentityState(v string) {
	o.IdentityState = &v
}

// GetNickName returns the NickName field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetNickName() string {
	if o == nil || IsNil(o.NickName) {
		var ret string
		return ret
	}
	return *o.NickName
}

// GetNickNameOk returns a tuple with the NickName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetNickNameOk() (*string, bool) {
	if o == nil || IsNil(o.NickName) {
		return nil, false
	}
	return o.NickName, true
}

// HasNickName returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasNickName() bool {
	if o != nil && !IsNil(o.NickName) {
		return true
	}

	return false
}

// SetNickName gets a reference to the given string and assigns it to the NickName field.
func (o *GetIdentityResponseV3) SetNickName(v string) {
	o.NickName = &v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetTags() []string {
	if o == nil || IsNil(o.Tags) {
		var ret []string
		return ret
	}
	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetTagsOk() ([]string, bool) {
	if o == nil || IsNil(o.Tags) {
		return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasTags() bool {
	if o != nil && !IsNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given []string and assigns it to the Tags field.
func (o *GetIdentityResponseV3) SetTags(v []string) {
	o.Tags = v
}

// GetValidatePayoutRails returns the ValidatePayoutRails field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetValidatePayoutRails() []string {
	if o == nil || IsNil(o.ValidatePayoutRails) {
		var ret []string
		return ret
	}
	return o.ValidatePayoutRails
}

// GetValidatePayoutRailsOk returns a tuple with the ValidatePayoutRails field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetValidatePayoutRailsOk() ([]string, bool) {
	if o == nil || IsNil(o.ValidatePayoutRails) {
		return nil, false
	}
	return o.ValidatePayoutRails, true
}

// HasValidatePayoutRails returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasValidatePayoutRails() bool {
	if o != nil && !IsNil(o.ValidatePayoutRails) {
		return true
	}

	return false
}

// SetValidatePayoutRails gets a reference to the given []string and assigns it to the ValidatePayoutRails field.
func (o *GetIdentityResponseV3) SetValidatePayoutRails(v []string) {
	o.ValidatePayoutRails = v
}

// GetVersion returns the Version field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetVersion() string {
	if o == nil || IsNil(o.Version) {
		var ret string
		return ret
	}
	return *o.Version
}

// GetVersionOk returns a tuple with the Version field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetVersionOk() (*string, bool) {
	if o == nil || IsNil(o.Version) {
		return nil, false
	}
	return o.Version, true
}

// HasVersion returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasVersion() bool {
	if o != nil && !IsNil(o.Version) {
		return true
	}

	return false
}

// SetVersion gets a reference to the given string and assigns it to the Version field.
func (o *GetIdentityResponseV3) SetVersion(v string) {
	o.Version = &v
}

// GetSchemaVersion returns the SchemaVersion field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetSchemaVersion() string {
	if o == nil || IsNil(o.SchemaVersion) {
		var ret string
		return ret
	}
	return *o.SchemaVersion
}

// GetSchemaVersionOk returns a tuple with the SchemaVersion field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetSchemaVersionOk() (*string, bool) {
	if o == nil || IsNil(o.SchemaVersion) {
		return nil, false
	}
	return o.SchemaVersion, true
}

// HasSchemaVersion returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasSchemaVersion() bool {
	if o != nil && !IsNil(o.SchemaVersion) {
		return true
	}

	return false
}

// SetSchemaVersion gets a reference to the given string and assigns it to the SchemaVersion field.
func (o *GetIdentityResponseV3) SetSchemaVersion(v string) {
	o.SchemaVersion = &v
}

// GetCreatedAt returns the CreatedAt field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetCreatedAt() time.Time {
	if o == nil || IsNil(o.CreatedAt) {
		var ret time.Time
		return ret
	}
	return *o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.CreatedAt) {
		return nil, false
	}
	return o.CreatedAt, true
}

// HasCreatedAt returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasCreatedAt() bool {
	if o != nil && !IsNil(o.CreatedAt) {
		return true
	}

	return false
}

// SetCreatedAt gets a reference to the given time.Time and assigns it to the CreatedAt field.
func (o *GetIdentityResponseV3) SetCreatedAt(v time.Time) {
	o.CreatedAt = &v
}

// GetUpdatedAt returns the UpdatedAt field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetUpdatedAt() time.Time {
	if o == nil || IsNil(o.UpdatedAt) {
		var ret time.Time
		return ret
	}
	return *o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetUpdatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.UpdatedAt) {
		return nil, false
	}
	return o.UpdatedAt, true
}

// HasUpdatedAt returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasUpdatedAt() bool {
	if o != nil && !IsNil(o.UpdatedAt) {
		return true
	}

	return false
}

// SetUpdatedAt gets a reference to the given time.Time and assigns it to the UpdatedAt field.
func (o *GetIdentityResponseV3) SetUpdatedAt(v time.Time) {
	o.UpdatedAt = &v
}

// GetIdentityType returns the IdentityType field value
func (o *GetIdentityResponseV3) GetIdentityType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.IdentityType
}

// GetIdentityTypeOk returns a tuple with the IdentityType field value
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetIdentityTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IdentityType, true
}

// SetIdentityType sets field value
func (o *GetIdentityResponseV3) SetIdentityType(v string) {
	o.IdentityType = v
}

// GetPaymentRole returns the PaymentRole field value
func (o *GetIdentityResponseV3) GetPaymentRole() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PaymentRole
}

// GetPaymentRoleOk returns a tuple with the PaymentRole field value
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetPaymentRoleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PaymentRole, true
}

// SetPaymentRole sets field value
func (o *GetIdentityResponseV3) SetPaymentRole(v string) {
	o.PaymentRole = v
}

// GetInternalId returns the InternalId field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetInternalId() string {
	if o == nil || IsNil(o.InternalId) {
		var ret string
		return ret
	}
	return *o.InternalId
}

// GetInternalIdOk returns a tuple with the InternalId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetInternalIdOk() (*string, bool) {
	if o == nil || IsNil(o.InternalId) {
		return nil, false
	}
	return o.InternalId, true
}

// HasInternalId returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasInternalId() bool {
	if o != nil && !IsNil(o.InternalId) {
		return true
	}

	return false
}

// SetInternalId gets a reference to the given string and assigns it to the InternalId field.
func (o *GetIdentityResponseV3) SetInternalId(v string) {
	o.InternalId = &v
}

// GetOriginatorAccountNumber returns the OriginatorAccountNumber field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetOriginatorAccountNumber() string {
	if o == nil || IsNil(o.OriginatorAccountNumber) {
		var ret string
		return ret
	}
	return *o.OriginatorAccountNumber
}

// GetOriginatorAccountNumberOk returns a tuple with the OriginatorAccountNumber field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetOriginatorAccountNumberOk() (*string, bool) {
	if o == nil || IsNil(o.OriginatorAccountNumber) {
		return nil, false
	}
	return o.OriginatorAccountNumber, true
}

// HasOriginatorAccountNumber returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasOriginatorAccountNumber() bool {
	if o != nil && !IsNil(o.OriginatorAccountNumber) {
		return true
	}

	return false
}

// SetOriginatorAccountNumber gets a reference to the given string and assigns it to the OriginatorAccountNumber field.
func (o *GetIdentityResponseV3) SetOriginatorAccountNumber(v string) {
	o.OriginatorAccountNumber = &v
}

// GetBusiness returns the Business field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetBusiness() BusinessIdentity {
	if o == nil || IsNil(o.Business) {
		var ret BusinessIdentity
		return ret
	}
	return *o.Business
}

// GetBusinessOk returns a tuple with the Business field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetBusinessOk() (*BusinessIdentity, bool) {
	if o == nil || IsNil(o.Business) {
		return nil, false
	}
	return o.Business, true
}

// HasBusiness returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasBusiness() bool {
	if o != nil && !IsNil(o.Business) {
		return true
	}

	return false
}

// SetBusiness gets a reference to the given BusinessIdentity and assigns it to the Business field.
func (o *GetIdentityResponseV3) SetBusiness(v BusinessIdentity) {
	o.Business = &v
}

// GetIndividual returns the Individual field value if set, zero value otherwise.
func (o *GetIdentityResponseV3) GetIndividual() IndividualIdentity {
	if o == nil || IsNil(o.Individual) {
		var ret IndividualIdentity
		return ret
	}
	return *o.Individual
}

// GetIndividualOk returns a tuple with the Individual field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetIdentityResponseV3) GetIndividualOk() (*IndividualIdentity, bool) {
	if o == nil || IsNil(o.Individual) {
		return nil, false
	}
	return o.Individual, true
}

// HasIndividual returns a boolean if a field has been set.
func (o *GetIdentityResponseV3) HasIndividual() bool {
	if o != nil && !IsNil(o.Individual) {
		return true
	}

	return false
}

// SetIndividual gets a reference to the given IndividualIdentity and assigns it to the Individual field.
func (o *GetIdentityResponseV3) SetIndividual(v IndividualIdentity) {
	o.Individual = &v
}

func (o GetIdentityResponseV3) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetIdentityResponseV3) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.IdentityId) {
		toSerialize["identityId"] = o.IdentityId
	}
	if !IsNil(o.IdentityState) {
		toSerialize["identityState"] = o.IdentityState
	}
	if !IsNil(o.NickName) {
		toSerialize["nickName"] = o.NickName
	}
	if !IsNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}
	if !IsNil(o.ValidatePayoutRails) {
		toSerialize["validatePayoutRails"] = o.ValidatePayoutRails
	}
	if !IsNil(o.Version) {
		toSerialize["version"] = o.Version
	}
	if !IsNil(o.SchemaVersion) {
		toSerialize["schemaVersion"] = o.SchemaVersion
	}
	if !IsNil(o.CreatedAt) {
		toSerialize["createdAt"] = o.CreatedAt
	}
	if !IsNil(o.UpdatedAt) {
		toSerialize["updatedAt"] = o.UpdatedAt
	}
	toSerialize["identityType"] = o.IdentityType
	toSerialize["paymentRole"] = o.PaymentRole
	if !IsNil(o.InternalId) {
		toSerialize["internalId"] = o.InternalId
	}
	if !IsNil(o.OriginatorAccountNumber) {
		toSerialize["originatorAccountNumber"] = o.OriginatorAccountNumber
	}
	if !IsNil(o.Business) {
		toSerialize["business"] = o.Business
	}
	if !IsNil(o.Individual) {
		toSerialize["individual"] = o.Individual
	}
	return toSerialize, nil
}

func (o *GetIdentityResponseV3) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"identityType",
		"paymentRole",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetIdentityResponseV3 := _GetIdentityResponseV3{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varGetIdentityResponseV3)

	if err != nil {
		return err
	}

	*o = GetIdentityResponseV3(varGetIdentityResponseV3)

	return err
}

type NullableGetIdentityResponseV3 struct {
	value *GetIdentityResponseV3
	isSet bool
}

func (v NullableGetIdentityResponseV3) Get() *GetIdentityResponseV3 {
	return v.value
}

func (v *NullableGetIdentityResponseV3) Set(val *GetIdentityResponseV3) {
	v.value = val
	v.isSet = true
}

func (v NullableGetIdentityResponseV3) IsSet() bool {
	return v.isSet
}

func (v *NullableGetIdentityResponseV3) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetIdentityResponseV3(val *GetIdentityResponseV3) *NullableGetIdentityResponseV3 {
	return &NullableGetIdentityResponseV3{value: val, isSet: true}
}

func (v NullableGetIdentityResponseV3) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetIdentityResponseV3) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


